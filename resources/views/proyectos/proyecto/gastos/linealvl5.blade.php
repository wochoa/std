@foreach($gastos as $id=>$d)
    @php($lvl=$class.'-'.$id)
<tr class="gradeU lvl_5 {{$lvl}}" data-level="{{$nivel}}" data-status="0" data-class="{{$lvl}}">
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td style="text-align:right">@if($d->monto_comp>0||$d->monto_dev>0)
            <button type="button" class="ejec_plus btn btn-xs btn-link glyphicon glyphicon-plus"></button>
    @endif
    </td>
    <td colspan="8">
        <strong>Certificado:</strong> {{$d->certificado}}<br>
        <strong>Monto:</strong> {{Money::toMoney($d->monto_cert)}}<br>
        @if(isset($d->dispositivo)&&$d->dispositivo!='')
        <strong>Dispositivo Legal:</strong> {{$d->dispositivo}}<br>
        @endif
        <strong>Concepto:</strong> {{$d->concepto}}</td>
    <td style="border-left: solid #000 2px;">{{($d->monto_cert>0)?Money::toMoney($d->monto_comp,''):''}}</td>
    <td>{{($d->monto_cert>0)?Money::toMoney($d->monto_cert-$d->monto_comp,''):''}}</td>
    <td>{{($d->monto_cert>0)?Money::porcentaje($d->monto_comp/$d->monto_cert):''}}</td>
    <td style="border-left: solid #000 2px;">{{Money::toMoney($d->monto_dev,'')}}</td>
    <td>{{($d->monto_cert>0)?Money::toMoney($d->monto_cert-$d->monto_dev,''):''}}</td>
    <td style="border-right: solid #000 2px;">{{($d->monto_cert>0)?Money::porcentaje($d->monto_dev/$d->monto_cert):''}}</td>
