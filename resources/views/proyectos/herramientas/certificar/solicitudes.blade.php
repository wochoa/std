...
@section('tabContent')
    <div class="panel-body" id="solicitudes_module">
...
@push('scripts')
<script>
    $('#solicitudes_module').saveModule({
        rList: '{{route('proyectos.herramientas.certificar.list',['proy'=>$proy,'anio'=>$anio])}}?listar=1',
        ...
        rPrint:"{{route('proyectos.herramientas.certificar.show',['proy'=>$proy,'anio'=>$anio,'id'=>'%s'])}}",
        token:'{{csrf_token()}}'

    });
    $(document).ready(function () {
        $('.print').click(function () {
            rute="{{route('proyectos.herramientas.certificar.show',['proy'=>$proy,'anio'=>$anio,'id'=>'%s'])}}";
            rute = rute.replace(/%s/g, $(this).data('id'));
            var ventimp=window.open(rute);
        })
    })
</script>
@endpush